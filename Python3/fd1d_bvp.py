...
#
#    function Aprime ( X ), evaluates a'(x)
#
#    function C ( X ), evaluates c(x)
#
#    function F ( X ), evaluates f(x)
#
#    real X(N), the mesh points, which may be nonuniformly spaced.
#
#  Output:
#
#    real U(N), the value of the finite difference approximation
#    to the solution.
#
  import numpy as np
#
...
#
  ...
  value = np.empty_like ( x )
  i1 = np.where ( x <= 1.0 / 3.0 )
  value[i1] = ( x[i1] + 3.0 * x[i1]**2 + 5.0 * x[i1]**3 + x[i1]**4 ) * np.exp ( x[i1] )
  i2 = np.where (      1.0 / 3.0 < x )
  value[i2] = ( - 1.0 + ( 10.0 / 3.0 ) * x[i2] + ( 43.0 / 9.0 ) * x[i2]**2 + x[i2]**3 ) * np.exp ( x[i2] )
