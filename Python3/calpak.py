...
#  Input:
#
#    integer Y, M, D, a year, month, and day representing a date.  
#
#  Output:
#
#    integer Y, M, D.  On M should have decreased by one month, 
#    and D gone up by the number of days in the month we "cashed in".  
#    Y may be affected if the input value of M was 1.
#
  while ( d <= 0 ):
...
#
#    integer Y, M, D, the YMD date.
#    D is guaranteed to be between 1 and the number of days in M.
#
...
#  Input:
#
#    integer Y1, M1, D1, the first date.
#
#    integer Y2, M2, D2, the second date.
#
  y = y1
  m = m1
  d = d1
...
#  Output:
#
#    integer Y, M, D, the "deflated" YMD date.
#
  if ( y == 1582 ):
    if ( m == 10 ):
      if ( 15 <= d ):
...
#  Input:
#
#    integer Y, the year, which must be 1583 or greater.
#    (The formula is only valid for years after the Gregorian calendar
#    was adopted.)
#
#  Output:
#
#    integer M, D, the month and day of Easter.
#
  if ( y <= 0 ):
    m = -1;
    d = -1;
...
#  Output:
#
#    integer M, D, the month and day of the Julian
...
#  Output:
#
#    integer M, D, the month and day of the Julian calendar
...
#  Output:
#
#    real JED, the Julian Ephemeris Date of the epoch.
#
  jed = 2289425.5
...
def epoch_to_jed_byzantine ( ):

#*****************************************************************************80
#
## epoch_to_jed_byzantine(): epoch of the Byzantine calendar as a JED.
#
#  Discussion:
#
#    The Byzantine calendar has the epoch 1 September 5509 BC.
#
#  Licensing:
#
#    This code is distributed under the GNU LGPL license.
...
#  Input:
#
#    integer C, integer W, real S,
#    the GPS cycle/week/second date.
#
#  Output:
#
#    real JED, the corresponding Julian Ephemeris Date.
#
...
#  Input:
#
#    integer Y, M, D, H, the year, month, day
...
#
#    integer Y, M, D, H, the year, month, day
#    and hour of the date after borrowing.
#
  while ( h <= 0 ):
...
#
#    integer Y, M, D, H, the year, month, day
#    and hour of the date.  On H is presumably 24 or greater.
#
#  Output:
#
#    integer Y, M, D, H, the year, month, day
#    and hour of the date, after carrying.
#
  while ( 24 < h ):
...
#  Output:
#
#    integer Y, M, D, the inflated YMD date.
#
  if ( y == 1582 ):
    if ( m == 10 ):
      if ( 5 <= d ):
...
#  Output:
#
#    logical VALUE, is TRUE if JED is legal, and FALSE otherwise.
#
  if ( 0.0 <= jed ):
    value = True
  else:
...
#  Input:
#
#    integer I, the test date requested.
#
#  Output:
#
#    real JED, the Julian Ephemeris Date.
...
#  Input:
#
#    real JED1, the Julian Ephemeris Date.
#
#  Output:
#
#    real JED2, the Julian Ephemeris Date
#    of the next noon.
#
  jed2 = round ( jed1 )
#
...
#  Output:
#
#    integer W, the day of the week of the date.
...
#  Output:
#
#    integer Y, M, D, real F, the YMDF date.
#
...
#  Input:
#
#    integer Y, M, D, H, N, the year,
#    month, day, hour and minute representing a date.  On N
#    might be negative.
#
#  Output:
#
#    integer Y, M, D, H, N, the year, month, day, hour and minute 
#    representing a date.  H should have decreased by one, and N 
#    gone up by 60.
#
  while ( n < 0 ):
...
#  Input:
#
#    integer Y, M, D, H, N, the date.
#
#  Output:
#
#    integer Y, M, D, H, N, the date, after carrying.
#    N is between 0 and 59.
#
  while ( 60 <= n ):
...
#  Input:
#
#    integer Y, M, the YM date. M might be negative.
#
#  Output:
#
#    integer Y, M, the YM date after borrowing.
#    Y should have decreased by one, and M gone up by the number 
#    of months in the year that we "cashed in".
#
  while ( m <= 0 ):
...
#  Input:
#
#    integer Y, M, the year and month.
#
#  Output:
#
#    integer Y, M, the year and month.  M is no greater than 12.
#
  while ( True ):
...
#  Input:
#
#    integer Y, the year in which the month occurred.
#
#    integer M, the number of the month.
#
#  Output:
#
#    integer DAYS, the number of days
#    in the month.
#
  import numpy as np
...
#  Input:
#
#    integer M, the month index.
#
#  Output:
#
#    string month_name, the month name.
#
  if ( m == 1 ):
    month_name = 'January'
  elif ( m == 2 ):
    ...
    raise Exception ( 'weekday_to_name_common - Fatal error!' )
...
#
  if ( m == 1 ):
    ...
    print ( '  Index M must be between 1 and 12.' )
    raise Exception ( 'weekday_to_name_common3 - Fatal error!' )
...
#  Output:
#
#    real JED, the JED for the current date.
#
  import time
...
#
  ...
  for test in range ( 0, test_num ):
    ...
    x = - 10.0 + 20.0 * np.random.rand ( )
    y = - 10.0 + 20.0 * np.random.rand ( )
...
#  Input:
#
#    integer Y, M, D, H, N, S, the YMDHMS date.
#
#  Output:
#
#    integer Y, M, D, H, N, S, the YMDHMS date after borrowing.
#
  while ( s < 0 ):
...
#  Input:
#
#    integer Y, M, D, H, N, S, the year, month, day, hours, minutes, seconds.
#
#  Output:
#
#    integer Y, M, D, H, N, S, the updated information after carrying.
#    S is between 0 and 59.
#
  while ( 60 <= s ):
...
#  Output:
#
#    real JED, the Julian Ephemeris Date of the date.
#
  jed = 2299160.5
...
#
  if ( w == 1 ):
    ...
    print ( '  Index W must be between 1 and 7.' )
    raise Exception ( 'weekday_to_name_common2 - Fatal error!' )
...
#  Input:
#
#    integer Y, the astronomical year.
#
#  Output:
#
#    integer Y2, the Common year.
#
  if ( y <= 0 ):
    y2 = y - 1
  else:
...
#  Input:
#
#    integer Y, the Common year.
#
#  Output:
#
#    integer Y2, the Astronomical year.
#  
  if ( y < 0 ):
    y2 = y + 1
  elif ( y == 0 ):
    ...
    raise Exception ( 'y_common_to_astronomical - Fatal error!' )
  else:
    y2 = y
...
#  Input:
#
#    integer Y, the Y date.
#
#  Output:
#
#    string S, a representation of the date.
#
  if ( 0 <= y ):
    s1 = 'CE'
  else:
...
#  Output:
#
#    logical VALUE, TRUE if the year was a leap year,
#    FALSE otherwise.
#
  if ( y == 0 ):
    print ( '' )
    ...
    raise Exception ( 'year_is_leap_common - Fatal error!' )
#
...
#  Output:
#
#    integer VALUE, the number of months in the year.
#
  value = 12
...
#  Input:
#
#    integer Y, the year.  The year 0 is illegal input.
#
#  Output:
#
#    integer E, the epact, between 0 and 28.
#
  if ( y == 0 ):
    print ( '' )
    print ( 'year_to_epact_julian - Fatal error!' )
    print ( '  Illegal input Y = 0.' )
    raise Exception ( 'year_to_epact_julian - Fatal error!' )
...
#  Output:
#
#    integer G, the golden number, between 1 and 19.  This
...
#
  ...
  if ( y == 0 ):
    ...
    print ( '  Illegal input Y = 0.' )
    raise Exception ( 'year_to_golden_number - Fatal error!' )
#
...
#  Input:
#
#    integer Y, M, the YM date.
#
#  Output:
#
#    integer Y, M, the YM date, after normalization.
#
#    integer IERROR, is 0 if no error was found in the date
#    and 1 otherwise.
#
...
#  Output:
#
#    integer Y, M, D, the YMD date after normalization.
#
#    integer IERROR, is 0 if no error was found in the date
#    and 1 otherwise.
#
...
#  Output:
#
#    integer W, is the week day number of the date, with
...
#  Input:
#
#    integer Y1, M1, D1, real F1, the
#    first YMDF date.
#
#    integer Y2, M2, D2, real F2, the
#    second YMDF date.
#
#  Output:
#
...
#  Input:
#
#    integer Y1, M1, D1, real F1, the YMDF date.
#
#  Output:
#
#    integer Y2, M2, D2, real F2, tomorrow's YMDF date.
#
  y2 = y1
  m2 = m1
  d2 = d1 + 1
...
#
  ...
  print ( '  Illegal date!' )
  raise Exception ( 'ymdf_to_jed_common - Fatal error!' )
...
#  Output:
#
#    real JED, the corresponding JED.
#
  import numpy as np
#
...
#  Input:
#
#    integer Y, M, D, double F, the YMDF date.
#
#  Output:
#
#    string S, a representation of the date.
...
#  Input:
#
#    integer Y, M, D, H, N, S, the date information.
#
#  Output:
#
#    integer Y, M, D, H, N, S, the normalized data information.
#
#    integer IERROR, is 0 if no error was detected in the
