...
def comp_random_grlex ( kc, rank1, rank2 ):
...
#
  rank = np.random.random_integers ( rank1, rank2 )
#
...
#
  ...
  for test in range ( 0, 5 ):
    xc, rank = comp_random_grlex ( kc, rank1, rank2 )
    nc = np.sum ( xc )
    print ( '   %3d: ' % ( rank ), end = '' )
    print ( '    %2d = ' % ( nc ), end = '' )
    for j in range ( 0, kc - 1 ):
...
#
  ...
  for test in range ( 0, 5 ):
    xc, rank3 = comp_random_grlex ( kc, rank1, rank2 )
...
#
  ...
  xlo = -1.0
  xhi = +1.0
  x = xlo + ( xhi - xlo ) * np.random.rand ( m, n )
